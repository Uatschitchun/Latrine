...
# Time - current GPS time (in format defined by user. You may choose from predefined styles or define you own by this specifica
ion)
# my $time = param('time') || '';
...
if ($key =~ /^[\d\w]{5,15}$/ ) {
        ...
        # Output latitude, longitude and other data as JSON
                ...
                if (open STORE, "< $store") {
                        ...
                        print qq ~ "lat":"$lat","lon":"$lon","alt":"$alt","speed":"$speed","bearing":"$bearing","acc":"$acc","t
me":"$time","now":"$now"~;
                        close STORE;
                } else {
                        print '"error":"Can\'t open filestore"';
